...
- Restore cryptographic test execution under the maintainer's production-secret
  prohibition. Gitleaks scans repository content; generated test keys remain
  permitted, and denial probes exercise the local execution gate.
- Pin agent-safe-spl to the existing lockfile revision so source installation
  does not depend on the upstream default branch. Document locked installation.
- Update compatible anyhow and rand patches to clear RustSec warnings; align
  checked division with current Clippy without changing policy thresholds.
